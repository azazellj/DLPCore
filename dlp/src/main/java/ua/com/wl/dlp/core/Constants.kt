package ua.com.wl.dlp.core

/**
 * @author Denis Makovskyi
 */

object Constants {

    const val META_APP_ID = "ua.com.wl.dlp.app_id"
    const val META_STAGING_URL = "ua.com.wl.dlp.staging_url"
    const val META_PRODUCTION_URL = "ua.com.wl.dlp.production_url"

    const val KOIN_NAMED_URL = "url"
    const val KOIN_NAMED_APP_ID = "app_id"

    const val HEADER_APP_ID = "X-APPLICATION-ID"
    const val HEADER_UNAUTHORIZED = "Unauthorized"
    const val HEADER_AUTHORIZATION = "Authorization"

    const val VALUE_PERMIT = "permit"
}